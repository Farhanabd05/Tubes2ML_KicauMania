...
class Softmax:
    ...
    def forward(self, inputs: np.ndarray) -> np.ndarray:
        # axis=-1 agar kompatibel dengan input (N, C) maupun (N, T, C)
        inputs_aman = inputs - np.max(inputs, axis=-1, keepdims=True)
        expi = np.exp(inputs_aman)
        self.output = expi / np.sum(expi, axis=-1, keepdims=True)
        return self.output
    ...
    def backward(self, d_values: np.ndarray) -> np.ndarray:
        ...
        for b in range(batch_size):
            ...
            jacobian = np.diagflat(self.output[b]) - np.dot(o, o.T)
            d_inputs[b] = np.dot(jacobian, d_values[b])
        return d_inputs
