...
def init_uniform(shape, lower_bound=0.0, upper_bound=1.0, seed=None):
    np.random.seed(seed)
    return np.random.uniform(lower_bound, upper_bound, size = shape)

def init_normal(shape, mean=0.0, variance=1.0, seed=None):
    np.random.seed(seed)
    return np.random.normal(mean, (variance)**(1/2), size=shape)

def init_xavier(shape, seed=None):
    np.random.seed(seed)
